...
- publication workflowはPRから実行せず、merge後のmainだけがrun-scoped
  ...
  credential境界込みで検証します。このPRにはimage evidence、resident ledger、
...
- 続く工程はimage-publication policy review/merge、main-only Polaris image
...
- Dependency evidence-only PR:
  ...
  (merged, `Refs #61`)
- Image publication policy PR: current focused PR (number assigned on creation,
  `Refs #61`)
- Runtime follow-up depends on: `#27` (closed prerequisite checkpoint)
- Execution order: `1 of 8`
- Queue: image-publication policy review/merge、main-only Polaris image
  publication、image evidence-only review、atomic Polaris/PostgreSQL admission、
