...
- 修復Draft PR #77は`cosign sign --bundle`によるbundle生成・保持を維持し、
  exact digest/identity/issuerを検証するregistry `cosign verify`からだけ
  `--bundle`を除きます。semantic auditはbundle再混入、sign側bundle欠落、
  identity/issuer/reference drift、bundle/verification evidence保持漏れを
  fail-closedで拒否します。
- 現在の実装範囲は`dependency_snapshot_publication_pending`です。main限定の
  ...
  要求します。
- GHCR packageはprivateがdefaultであり、その場合は署名・provenance生成後の
  anonymous pullで意図的に停止し得ます。そのattemptはadmitせず、ownerが公開の
  不可逆性を確認してpackageをpublic化した後にrerunします。今回のpackageは
  package pageと空registry configによるmanifest/config/two-layer取得および
  signature verifyで既にPublicと確認できたため、visibility変更は不要です。
...
- ORAS relative-layer path repair PR:
  ...
  (merged, `Refs #61`)
- Cosign registry verification repair Draft PR:
...
- Queue: PR #77 Cosign registry verification修復のreview/merge、新しいmain
  run、dependency evidence-only review、main-only Polaris image publication、
