...
- 修復PR #75は、`files-2.1`をGAV、filename、SHA-256が完全一致する
  ...
  protected main runのpublication evidenceではありません。implementation commit
  `ec1d8a4`ではCIとSupply chain security checkが成功し、Codex reviewもmajor
  issueなしで完了しており、ownerのmerge承認待ちです。
...
- Gradle verified-closure projection repair PR:
  ...
  (CI/Codex review済み、merge承認待ち, `Refs #61`)
- Runtime follow-up depends on: `#27` (closed prerequisite checkpoint)
- Execution order: `1 of 8`
- Queue: PR #75 verified-closure projection repairのmerge、新しいmain run、
