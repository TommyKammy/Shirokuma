...
are recorded in its row. Their retained evidence is authoritative for this
evidence review only. Atomic admission must preflight anonymous exact-digest
availability and rerun both exact-image and CycloneDX-input scans with each
database no more than 24 hours old, preserving the complete 56 Wolfi plus four
Go library coverage.
