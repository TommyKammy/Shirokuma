...
|---|---|---|---|---|---|---|---|---|
| Trino | `483` (2026-07-17) | blocked upstream index `docker.io/trinodb/trino@sha256:db58cc93e593a2706553745f276bb119c9810e69918be56ecde088ba7ccb0534`; mainline ADR-0022 repository-owned source-build candidate; sequence 8 is consumed and publication is reblocked | The upstream index contains `linux/amd64`, `linux/arm64`, and `linux/ppc64le`; the arm64 child is `sha256:aa18e61b2e7776ab8641ba8baaa8687d0430894e88c639e61010cc46a994ab36`. The contract pins Maven/Temurin builder index `sha256:7e461cec477077c1d9e50b13df8aef9018764410f4c4cd7c34803f10c4c99e4c` with native arm64 child `sha256:5476bfca9d0a6485b7161f6863123f7e6822336de4177273b47b5ec38ffd573a`, plus Corretto index `sha256:32d81edae73e1670244827c2f12e5bcf0d335f035b538455fe9d02eb0771d41b` and arm64 child `sha256:da20e1e0a2004dfb95e963d6ad978b5c0effdfc7000bce6a68836058ef24b427`. | Apache-2.0 | The index has no attestation manifest or trusted signer; both annotated tag object `32d4f28e8311ea6f67edca209df59a0493d869fa` and source commit `50b0b50b75abd47f830b7805ee1b51716eb4065e` are unsigned; no trusted SLSA provenance binds the exact source coordinates to an approved publisher identity. | mainline ADR-0022 source-build candidate; `dependency_snapshot_publication_reauthorization_pending`. Sequence 8 run `33477757991` validated the exact remediation at raw High=0/Critical=0, then failed closed with `INDEPENDENT_REVIEW` before candidate download or registry authentication because the OWNER final-head attestation was missing at the write-capable boundary. No dependency artifact was published. Rerun, another sequence-8 run, and a later sequence are unauthorized. The 476 fallback remains rejected at Critical=2/High=52. Dependency admission, image publication, resident admission, Flux, and runtime remain blocked. | Owner: WP-L1-QUERY-001 / Issue #63. Risk: the exact source publisher remains unauthenticated; SHA pinning or Shirokuma re-signing does not establish upstream identity. Controls: exact predecessor/final-head binding, consumed one-shot authorization, no rerun or renewal, synthetic/PoC data only, no public exposure, and raw High=0/Critical=0 without OpenVEX or waiver. Replace: authenticate the source or adopt a 480-or-newer release, then complete the repository-owned reproducible source build. | [483 release](https://github.com/trinodb/trino/releases/tag/483), [ADR-0033](../07_ADR/ADR-0033_Activate_exact_Trino_483_dependency_publication_sequence_8.md), [sequence 8 run](https://github.com/TommyKammy/Shirokuma/actions/runs/33477757991), [PR #157 feasibility run](https://github.com/TommyKammy/Shirokuma/actions/runs/33026390700), [license](https://github.com/trinodb/trino/blob/483/LICENSE) |
...
The signed-distribution fallback was reviewed separately on 2026-07-23. Trino
476 has a detached Maven Central signature that PGPy verifies against key
fingerprint `C328250FE23A2420814521EC0EB69F76FD171538`, but that key is not
anchored to a separately approved Trino trust root. A Syft 1.46.0 CycloneDX
scan followed by Trivy 0.72.0 reported Critical=2 and High=52, including
`CVE-2026-34214` in the required `io.trino:trino-iceberg` 476 module, fixed in
480. Critical findings are never exception-eligible, so 476 is rejected rather
than admitted as a fallback. The normalized result is
`bootstrap/trino/v476/feasibility.json`; RES-107 contains the complete review.

ADR-0022 completed a conditional source-build decision on 2026-07-22. GitHub
reports both the exact tag object and source commit as unsigned. ADR-0031 carries
that residual source-identity risk for the exact coordinates and bounded
`mac-studio-solo/local-lite` PoC through `2026-09-22T23:00:43Z`. Sequence 6 run
`32315191436` remains consumed after failing closed with `BUN_SNAPSHOT_IDENTITY`.
Fresh byte-identical Bun snapshot identities are manifest
...
Sequence 7 run `32786095668` is consumed after `MAVEN_SCAN_FINDING` reported
Critical=0/High=3. PR #157 exact-head run `33026390700` subsequently proved
the focused Netty 4.2.17.Final and source-reconstructed docker-java transport
remediation with raw High=0/Critical=0. Sequence 8 run `33477757991`, attempt
`1`, completed all source reconstruction, closed-build, offline-build, and raw
High=0/Critical=0 validation, then failed closed with `INDEPENDENT_REVIEW`
because the OWNER final-head attestation was missing at the write-capable
publication boundary. The retained candidate is read-only; candidate download,
registry authentication, registry write, and publication were not reached.
Sequence 8 is consumed, publication is reblocked, and rerun or a later sequence
requires a fresh explicit OWNER decision. Dependency admission and every
downstream boundary remain forbidden. The build design selects native-arm64
Maven/Temurin builder index
...
the dependency snapshot, network-none offline build, native container smoke,
image evidence, resident admission, and runtime remain separate fail-closed
checkpoints. The authorization cannot renew automatically, cannot stack with a
Trino vulnerability exception, and fails closed at expiry.
