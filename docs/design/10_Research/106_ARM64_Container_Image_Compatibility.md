...
| Polaris Admin Tool | `1.6.0` companion application (2026-07-09) | repository-owned source build is pending; the admitted `shirokuma-polaris` digest is server-only and cannot be reused as the Admin Tool | No `linux/arm64` image evidence is claimed. The next checkpoint publishes only a self-contained Gradle build-input superset on `ubuntu-24.04-arm`; image publication and smoke remain separate. | Apache-2.0 | The ASF PGP/SHA-512 source and reviewed server dependency OCI are the parent inputs. Admin tasks, a fresh network-none/offline/strict build, new signature, provenance, SBOM, scan, and image smoke remain mandatory before admission. | mainline prerequisite for relational JDBC bootstrap; build inputs are publication-pending, while Admin image, resident admission, credentials, and runtime remain blocked. | Owner: WP-L1-LAKE-002. Risk: upstream Admin dependencies include NoSQL/MongoDB in addition to PostgreSQL, and treating a build-input snapshot as an admitted relational-only image would expand the runtime boundary. Replace: keep catalog activation blocked until a separately reviewed Admin image passes the exact ARM64 and supply-chain gates. | [release](https://github.com/apache/polaris/releases/tag/apache-polaris-1.6.0), [Admin Tool](https://polaris.apache.org/releases/1.6.0/admin-tool/), [JDBC metastore](https://polaris.apache.org/releases/1.6.0/metastores/relational-jdbc/) |
