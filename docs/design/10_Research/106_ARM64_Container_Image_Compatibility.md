...
| Polaris Admin Tool | `1.6.0` companion application (2026-07-09) | pending repository-owned publication to `ghcr.io/tommykammy/shirokuma-polaris-admin:1.6.0-arm64` through `.github/workflows/polaris-admin-arm64.yml` and `bootstrap/polaris/v1.6.0/Containerfile.admin`; the admitted server digest is not reused | PR #87 approved the exact public Admin dependency superset for image build. The pending policy fixes the native arm64 Corretto runtime, upstream fast-jar layout, non-root `10000:10001`, and `--help` CLI smoke, but no Admin image digest is claimed before main publication and evidence review. | Apache-2.0 | ASF PGP/SHA-512 source, exact dependency OCI `ghcr.io/tommykammy/shirokuma-polaris-admin-gradle-dependencies@sha256:7a505defcd78c7a7b978e88cd4c72e0a5d8b69cbb57ddd311c163b09fe789d18`, fresh network-none/offline/strict build, keyless signature, provenance, SBOM, scan, and exact CLI smoke are required. | `admin_image_publication_pending`, next `admin_image_evidence_review_pending`; image admission, credentials, runtime, and Flux remain blocked. | Owner: WP-L1-LAKE-002. Risk: upstream Admin dependencies and image include NoSQL/MongoDB in addition to PostgreSQL, and a mutable tag or successful help smoke is not resident admission. Replace: keep catalog activation blocked until the exact published digest and evidence pass separate review and admission. | [release](https://github.com/apache/polaris/releases/tag/apache-polaris-1.6.0), [Admin Tool](https://polaris.apache.org/releases/1.6.0/admin-tool/), [JDBC metastore](https://polaris.apache.org/releases/1.6.0/metastores/relational-jdbc/) |
...
The next Admin image publication PR adds
