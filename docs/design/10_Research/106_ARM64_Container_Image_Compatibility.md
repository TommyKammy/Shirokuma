...
reports both the exact tag object and source commit as unsigned. ADR-0030 carries
that residual source-identity risk for the exact coordinates and bounded
`mac-studio-solo/local-lite` PoC through `2026-09-17T02:15:58Z`. Sequence 6 run
`32315191436` consumed its authorization and failed closed with
`BUN_SNAPSHOT_IDENTITY`. Publish was skipped, no artifact was retained, and
registry authentication or write was never reached. Rerun and sequence 7 are
unauthorized pending fresh exact evidence and a new OWNER decision. The build
