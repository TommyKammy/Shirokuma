...
- Flux CLIと配布マニフェストは`fluxcd/flux2`の安定リリースを固定する。初期採用版は`v2.9.1`とし、`main`または実行時の`latest`を追跡しない。
...
- controller imageはdigestを記録し、署名、SBOM、provenance、ARM64対応、High/Critical脆弱性をbootstrap前に検証する。
