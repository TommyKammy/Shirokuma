...
class KyvernoBootstrapContractTests(unittest.TestCase):
    def test_local_lab_admission_selects_only_the_seven_noncritical_images(self) -> None:
        ...
        admitted = [images[component] for component in admission["components"]]
        self.assertTrue(all(image["scan_summary"]["critical"] == 0 for image in admitted))
        self.assertEqual(
            admission["excluded"],
            {
